...
// jobs.photos ile aynı desen — bkz. app/api/jobs/[id]/photos/route.ts.

const ALLOWED_TYPES = ["image/jpeg", "image/png", "image/webp"];
const MAX_SIZE = 8 * 1024 * 1024;
const PREFIX = "servis-fotograflari/";
...
export async function POST(req: Request, { params }: Ctx) {
  ...
  const [ticket] = await db
    .select({ id: serviceTickets.id, photos: serviceTickets.photos })
    .from(serviceTickets)
  ...
  try {
    ...
    const photos = [...((ticket.photos as PhotoRef[]) ?? []), { id: photoId, url: blob.url }];
    await db.update(serviceTickets).set({ photos }).where(eq(serviceTickets.id, numericId));
...
export async function DELETE(req: Request, { params }: Ctx) {
  ...
  const photos = (ticket.photos as PhotoRef[]) ?? [];
