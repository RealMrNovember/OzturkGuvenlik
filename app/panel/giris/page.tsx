...
export default function PanelGirisPage() {
  ...
  const submitCode = async (e: FormEvent) => {
    ...
    try {
      ...
      const body = await res.json();
      if (!res.ok) {
        setError(body.error ?? "Kod doğrulanamadı");
        setLoading(false);
        return;
      }
      router.push("/panel");
        ...
        {!needsTwoFactor ? (
          ...
          >
            <div className="mb-4 flex items-center gap-2.5">
              ...
              <div>
                <p className="text-sm font-bold text-ink">İki adımlı doğrulama</p>
            ...
            <label className="block">
              <span className="mb-1.5 block text-xs font-semibold text-ink/70">Kod</span>
              <input
                type="text"
                required
                autoFocus
            ...
            <p className="mt-1.5 text-xs text-ink/40">
              Telefonunuza erişemiyorsanız, kayıtlı yedek kodlarınızdan birini girebilirsiniz.
            </p>
            ...
            <button
              ...
              className="mt-4 block w-full text-center text-xs font-semibold text-ink/50 hover:text-brand"
            >
              ← Geri dön
            </button>
          </form>
